...
import { sendDeploymentNotification } from './emailService.js';
import { verifyGithubSignature } from './validateGithubSignature.js';

export const handleGitHubWebhookRequest = (req, res) => {
  try {
    ...
    bashChildProcess.on('close', async (code) => {
      ...
      '-n', // Don't allocate a pseudo-terminal
      'safemystuff',
      'pm2 reload backend --update-env'
    ], {
      detached: true,
      stdio: 'pipe', // Use pipe instead of ignore
